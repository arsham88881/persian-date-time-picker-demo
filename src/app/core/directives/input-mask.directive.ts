...
export class DateMaskDirective {
  ...
  parseFormat() {
    if (this.disableInputMask) return;
  ...
  onInput(event: Event) {
    ...
    for (let i = 0; i < this.parts.length; i++) {
      ...
      if (formattedPart.length >= expectedLength) {
        formattedPart = this.validatePart(
          formattedPart.slice(0, expectedLength),
          this.parts[i],
        );
  ...
  onKeyDown(event: KeyboardEvent) {
    ...
    if (isTimeDelimiter) {
      if (this.delimiters[currentPartIndex - 1] === ':') {
        ...
        const currentPart = this.validatePart(
          parts[currentPartIndex - 1],
          this.parts[currentPartIndex - 1],
        );
        ...
        const newValue =
          parts
            ...
            .join(this.delimiters[currentPartIndex - 1]) + ':';
        input.value =
          newValue +
          parts.slice(currentPartIndex).join(this.delimiters[currentPartIndex]);
  ...
  validatePart(value: string, format: string): string {
    ...
    switch (type) {
      case 'h': // 12-hour format
        if (format[0] == 'H')
          return Math.min(Math.max(numValue, 0), 23)
            .toString()
            .padStart(2, '0');
      ...
      case 'm': // month or minute
        if (format === 'MM') {
          return Math.min(Math.max(numValue, 1), 12)
            .toString()
            .padStart(2, '0');
        }
