  ...
  implements ControlValueAccessor, OnInit, OnDestroy, OnChanges
{
  ...
  save(close = true): void {
    ...
    if (valueChanged) {
      ...
      this.form!.get('timeInput')?.setValue(this.formatTime(normalizedDate), {
        emitEvent: false,
      });
  ...
  isTimeDisabled(testDate: Date): boolean {
    ...
    return this.disabledTimesFilter
      ? this.disabledTimesFilter(testDate)
      : false;
  }
  ...
  scrollToSelectedItem(
    ...
    return new Promise((resolve) => {
      ...
      this.timeoutId = window.setTimeout(() => {
        const selectedElement = this.popupWrapper?.nativeElement.querySelector(
          `#selector_${id}`,
        );
        if (selectedElement) {
