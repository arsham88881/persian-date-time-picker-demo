  ...
  implements OnInit, OnChanges, AfterViewInit, OnDestroy
{
  ...
  generateCalendar(): void {
    ...
    this.days = Array.from({ length: 42 }, (_, i) =>
      this.dateAdapter!.addDays(startDate, i),
    );
  }
  ...
  updateRangeDateTime(timeDate: Date): void {
    if (this.activeInput === 'start' && this.selectedStartDate) {
      const updatedDate = this.applyTimeToDate(
        this.selectedStartDate,
        timeDate,
      );
  ...
  handleRangeSelection(date: Date): void {
    ...
    if (
      prevStartDate !== this.selectedStartDate ||
      prevEndDate !== this.selectedEndDate
    )
      this.changeDetectorRef.markForCheck();
  }
  ...
  isYearRangeDisabled(yearRange: YearRange): boolean {
    if (
      this.minDate &&
      this.dateAdapter!.getYear(this.minDate!)! > yearRange.end
    )
      return true;
    if (
      this.maxDate &&
      this.dateAdapter!.getYear(this.maxDate!)! < yearRange.start
    )
      return true;
