  ...
  implements ControlValueAccessor, OnInit, OnChanges, AfterViewInit, OnDestroy
{
  ...
  >;
  @ViewChild(DatePickerPopupComponent)
  ...
  onDateRangeSelected(dateRange: DateRange): void {
    ...
    const startFormatted = this.dateAdapter!.format(
      this.selectedStartDate!,
      this.format,
    );
    ...
    if (dateRange.end) {
      ...
      const endFormatted = this.dateAdapter!.format(
        this.selectedEndDate!,
        this.format,
      );
  ...
  clampDate(date: Date): Date | null {
    ...
    if (
      this.minDate &&
      this.dateAdapter!.isBefore(adjustedDate, this.minDate)
    ) {
      return this.minDate;
    }
  ...
  onFocusout(event: FocusEvent): void {
    ...
    if (
      ...
      !this.datePickerPopup?.elementRef.nativeElement.contains(
  ...
  handleRangeCorrectedValue(
    ...
    if (inputType === 'start') {
      ...
      this.selectedStartDate = this.dateAdapter!.parse(
        correctedValue,
        this.format,
      );
    } else {
      this.form!.get('endDateInput')?.setValue(correctedValue);
      this.selectedEndDate = this.dateAdapter!.parse(
        correctedValue,
        this.format,
      );
    }
